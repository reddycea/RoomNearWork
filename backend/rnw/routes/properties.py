...
from backend.rnw.services.geolocation_service import (
    bounding_box,
    filter_properties_by_distance,
)
from backend.rnw.services.google_maps_service import (
    attach_commute_measures,
    geocode_address,
    geocode_place_id,
...
from backend.rnw.utils.uploads import (
...
def _save_property_asset(
    ...
    existing_primary = any(
        asset.kind == "photo" and asset.is_primary
...
def _tenant_visible_property_query():
    """
    Tenant search must only show rentals when:
    - property is active
    - property status is available
    - landlord is approved
    - landlord has active landlord subscription
    """
...
def _populate_property_from_form(prop: Property) -> None:
    ...
    if place_id:
        geocoded = geocode_place_id(place_id, exact_address)
    elif exact_address:
    ...
    prop.approximate_address = (
        clean_user_text(request.form.get("approximate_address"), 255)
        or (geocoded.area_label if geocoded else None)
    )
    ...
    prop.nearest_transport = clean_user_text(
        request.form.get("nearest_transport"),
        160,
    )
    prop.commute_notes = clean_user_text(
        request.form.get("commute_notes"),
        1000,
    )
...
def _handle_asset_uploads(prop: Property) -> None:
    ...
    proof = _save_property_asset(
        prop,
        request.files.get("proof_registration"),
        "proof_registration",
        private=True,
    )

    id_doc = _save_property_asset(
        prop,
...
@properties_bp.get("")
def index():
    query = _tenant_visible_property_query()
    ...
    max_distance = (
        request.args.get("max_distance", type=float)
        or current_app.config.get("DEFAULT_SEARCH_RADIUS_KM", 20)
    )

    max_travel_minutes = (
        request.args.get("max_travel_minutes", type=int)
        or current_app.config.get("DEFAULT_MAX_TRAVEL_MINUTES", 45)
    )
    ...
    if workplace_address or workplace_place_id:
        if workplace_place_id:
            origin = geocode_place_id(workplace_place_id, workplace_address)
        else:
            origin = geocode_address(workplace_address)
        ...
        if origin.latitude is not None and origin.longitude is not None:
            ...
            if max_travel_minutes:
                properties = [
                    ...
                    if not getattr(prop, "commute_modes", None)
                    or any(
                        mode.duration_min is not None
                        and mode.duration_min <= max_travel_minutes
                        for mode in prop.commute_modes
                    )
                ]
            ...
            properties.sort(
                key=lambda prop: min(
                    [
                        mode.duration_min
                        for mode in getattr(prop, "commute_modes", [])
                        if mode.duration_min is not None
                    ]
                    or [999999]
                )
            )
    ...
    return render_template(
        ...
        google_maps_browser_key=current_app.config.get(
...
def detail(property_id: int):
    ...
    can_view_inactive = (
        ...
        and (
            current_user.id == property_.landlord_id
            or current_user.is_admin
        )
    )

    if not property_.is_active and not can_view_inactive:
        abort(404)
    ...
    if current_user.is_authenticated:
        ...
        user_review = RentalReview.query.filter_by(
    ...
    if current_user.is_authenticated and current_user.role == "tenant":
        can_apply, reason, tenant_subscription, used, limit = (
...
def create():
    if request.method == "POST":
        ...
        flash(
            "Listing submitted for review. Admin will check your photos, "
            "property registration proof and ID document.",
            "success",
        )
...
def edit(property_id: int):
    ...
    if request.method == "POST":
        ...
        flash(
            "Listing updated and sent for review if needed.",
            "success",
        )
...
def media(asset_id: int):
    ...
    if asset.is_private:
        ...
        if not (
            ...
            or current_user.id == prop.landlord_id
        ):
            abort(403)
...
def apply(property_id: int):
    ...
    if not property_.is_active or property_.status != "available":
        flash(
            "This rental is not available for applications.",
            "warning",
        )
    ...
    if not property_.landlord.can_act_as_landlord:
        flash(
            "This landlord is not approved yet.",
            "warning",
        )
    ...
    if not landlord_has_active_subscription(property_.landlord_id):
        flash(
            "This rental is not accepting applications right now.",
            "warning",
        )
